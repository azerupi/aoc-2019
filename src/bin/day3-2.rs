...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cross: Vec<_> = w1.find_crossings(w2);
    cross.sort_by_key(|p| p.manhattan_distance(&origin));

    let shortest = cross.iter()
        .map(|p| w1.steps_to_point_in_path(*p) + w2.steps_to_point_in_path(*p))
        .min()
        .unwrap();
...
impl LineSegment {
    ...
    fn is_point_on_segment(&self, p: Point) -> bool {
        self.is_point_in_x_range(p) && self.is_point_in_y_range(p)
    }
    ...
    fn length(&self) -> i32 {
        self.p.manhattan_distance(&self.q)
    }
}
...
impl Wire {
        ...
        for segment1 in &self.path {
            for segment2 in &other.path {
                match segment1.intersects(segment2) {
                    Some(p) if p != Point::origin() => intersections.push(p),
                    _ => {}
                }
            }
        }
    ...
    fn steps_to_point_in_path(&self, p: Point) -> i32 {
        let mut steps = 0;
        for s in &self.path {
            if s.is_point_on_segment(p) {
                steps += s.p.manhattan_distance(&p);
                break
            } else {
                steps += s.length();
            }
